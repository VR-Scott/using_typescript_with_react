...
const groupEventsByDay = (events: UserEvent[]) => {
  ...
  events.forEach((event) => {
    let rotDate = new Date(event.dateStart);
    const dateEndKey = createDateKey(new Date(event.dateEnd));
    let rotDateKey = createDateKey(rotDate);

    addToGroup(rotDateKey, event);

    while (rotDateKey !== dateEndKey) {
      rotDate.setDate(rotDate.getDate() + 1);
      rotDateKey = createDateKey(rotDate);
      addToGroup(rotDateKey, event);
    }
  });
...
const Calendar: React.FC<Props> = ({ events, loadUserEvents }) => {
  // load events when comp mounts we use useEffect()
  // MT dependency list called only after 1st render.
  // call loadUserEvents dispatch () ;
  useEffect(() => {
    loadUserEvents();
  }, []);
